...
const forbiddenCopy = [
  ...
  /만족도(?:가|를)?\s*(?:높|향상)/,
  /고급스럽게\s*(?:바뀌|변하)/,
  /사용자?\s*편의(?:가|를)?\s*(?:향상|개선)/,
  /(?:위생|안전)(?:적인)?\s*(?:환경|상태)?(?:이|가|을)?\s*(?:확보|향상|개선)/,
  /합리적인\s*비용/,
  /전문\s*(?:업체|시공)/,
  /(?:^|[.!?]\s*)(?:문|문제|작업|결과)\s*:/,
  ...
  const response = await fetchImpl(OPENAI_RESPONSES_URL, {
    ...
    body: JSON.stringify({
      ...
      instructions: [
        ...
        "강력, 확실, 완전, 만족도 향상, 안전·위생·편의 개선처럼 효과를 보장하거나 주관적으로 평가하는 표현은 쓰지 마세요.",
        "결과는 물내림 테스트, 누수 점검, 흔들림 확인, 작동 확인처럼 현장에서 실제로 확인한 행동과 상태로 표현하세요.",
